...
function SavingItem({ id, title, amount, date, category, description, deleteItem, indicatorColor, type }) {
  const [isModalOpen, setModalOpen] = useState(false);
  const [isChatOpen, setChatOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [recommendations, setRecommendations] = useState("");
  const [request, setRequest] = useState("");
  const { updateSaving, setError, getSavings } = useGlobalContext();
  ...
  const handleUpdate = async (updatedItem) => {
    try {
      const result = await updateSaving(updatedItem);
      Swal.fire({
        title: "Success!",
        text: "Saving updated successfully",
        icon: "success",
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/v1/get-saving-recommendation/${id}`,
        {
          headers: {
